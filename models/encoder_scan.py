...
from models.backbone.backbone_multiview import BackboneMultiview
...
class Encoder(nn.Module):
    def __init__(self, cfg, use_depth=False):
        ...
        if self.cfg.depth_em:
            ...
            self.depth_estimation.load_state_dict(torch.load('./save/scannet_depth_estimation_v8.pth', map_location='cpu'),
                                                  strict=True)
        ...
        self.projection = nn.Conv2d(self.cfg.out_channel, self.cfg.nb_class, 1)

        gaussian_sem_channels = 4 + 3 + 4 + self.cfg.nb_class + 1
        ...
        gaussian_rgb_channels = 4 + 3 + 4 + 75 + 1
        in_ch = self.d_feature
        if self.cfg.rgb_add:
            in_ch = in_ch + 3
    ...
    def create_gaussian(
        ...
        offset, scales, rotations, colors = raw_gaussians.split((4, 3, 4, color_num), dim=-1)
        ...
        if self.cfg.offset_dim>0:
            offset_sig = (offset[..., 0].sigmoid() > 0.5).float()
            means = means + offset[..., 1:].sigmoid() * depths[..., None] / self.cfg.far * offset_sig[..., None]
    ...
    def forward(self, batch: dict):
        ...
        if self.cfg.depth_add:
            if self.use_depth:
                ...
                cat_depths = rearrange(depth_map['level_2'], "b v h w -> (b v) () h w")
            trans_features = torch.cat((trans_features, cat_depths), dim=1)
        sem_2d_feats = self.semantic_up(trans_features)
        sem_gaussian_feats = self.to_sem_gaussians(sem_2d_feats)  # (bv,c,h,w)
        sem_raw_gaussians = rearrange(sem_gaussian_feats, "(b v) c h w -> b v (h w) c", b=b, v=v)
        sem_2d_logits = rearrange(self.projection(sem_2d_feats), "b c h w -> b h w c")
        ...
        means, covariances, colors, opacities = self.create_gaussian(
            rearrange(ref_extrinsics, "b v i j -> b v () () () i j"),  # (B,v,1,1,1,4,4)
            rearrange(ref_intrinsics, "b v i j -> b v () () () i j"),  # (B,v,1,1,1,3,3)
            rearrange(xy_ray, "b v r srf xy -> b v r srf () xy"),  # (B,v,h*w,1,1,2)
        ...
        means, covariances, sh, opacities = self.create_gaussian(
            rearrange(ref_extrinsics, "b v i j -> b v () () () i j"),  # (B,v,1,1,1,4,4)
            rearrange(ref_intrinsics, "b v i j -> b v () () () i j"),  # (B,v,1,1,1,3,3)
            rearrange(xy_ray, "b v r srf xy -> b v r srf () xy"),  # (B,v,h*w,1,1,2)
            rearrange(ref_depths, "b v h w -> b v (h w) () ()"),
        ...
        return sem_gs, rgb_gs, sem_2d_feats, sem_2d_logits, depth_map, ref_depths
